...
if(isset($_POST['SubmitButton']))
		{
		$pomoc = 1;
		$id_ex= $obiekt->getid(); //id tego co tworzy
		if(empty($_POST['Link_Google']))
		{
		$id_projektu = dodajProjekt($_POST['Nazwa_Projektu'],$_POST['data_roz'],$id_ex);
		}
		else
		{	
	$id_projektu = dodajProjekt($_POST['Nazwa_Projektu'],$_POST['data_roz'],$id_ex,$_POST['Link_Google']);
		}	
		}
	else if(isset($_POST['SubmitButtonZadanie']))
	{
		
		$pomoc = 2;
		$id_zadania = dodajZadanie($_POST['Nazwa_Zadania']);
		$id_projektu = $_SESSION['id_projektu'];

	}
	else if(isset($_GET['tryb']))
	{
		$pomoc = 3;
		$id_projektu = $_SESSION['id_projektu'];
	}
...
<script src="./style/js/repeater.js" type="text/javascript"></script>
	...
	<body>
		<nav class="navbar navbar-expand-md navbar-dark bg-dark sticky-top">
			...
			<li class="nav-item">
				...
				<div class="container">
			        <div style="width:100%; max-width: 600px; margin:0 auto;">
							...
							<?php
							if($pomoc == 1 or $pomoc == 3)
							{
								echo "<div class='card-header'>Przypisz Pracowników do Projektu</div>";
							}
							else
							{
								echo "<div class='card-header'>Przypisz Pracownika do Zadania</div>";
							}
							...
							?>
								...
								<div class="card-body">
									<span id="success_result"></span>
										...
										<button type="button" class="btn btn-primary repeater-add-btn">Dodaj Kolejnego Pracownika</button>
										</div>
										...
										<div class="form-group">
										  <div class="row">
										   <div class="col-md-9">
													<label>Wybierz Pracownika (puste pole oznacza brak użytkowników którzy nie są już przypisani)</label>
                                                <select class="form-control" data-skip-name="true" data-name="pracownicy[]" required>
                                               <?php 
												...
												if($pomoc == 1)
												{
												$zapytanie = $polaczenie->prepare("select *,Concat(Imie,' ',Nazwisko) as Dane from uzytkownicy where Poziom_Uprawnien = 1 or Poziom_Uprawnien = 2");
												$zapytanie -> execute();
												while($data = $zapytanie->fetch(PDO::FETCH_ASSOC))
												{
												...
												{
													$zapytanie = $polaczenie->prepare("select DISTINCT u.ID_Uzytkownika, Concat(u.Imie,' ',u.Nazwisko) as Dane from uzytkownicy u where (u.Poziom_Uprawnien = 1 or u.Poziom_Uprawnien = 2) and not exists(select * from projekty_uzytkownicy pu where pu.ID_Uzytkownika=u.ID_Uzytkownika and pu.ID_Projektu = $id_projektu)");
												$zapytanie -> execute();
												while($data = $zapytanie->fetch(PDO::FETCH_ASSOC))
												{
												echo "<option value='" . $data['ID_Uzytkownika'] . "' >". $data['Dane'] . "</option>";
							...
							<div class="clearfix"></div>
								<?php
									...
									<span class='validity'></span>";
														echo "<input type='text' name='zadania' value = '" . $id_zadania . "' hidden>";
												}
												else if($pomoc == 3)
												{
													echo "<input type='text' name='pomoc3' value = '" . $pomoc . "' hidden>";
			...
			<?php
				if($pomoc == 3 or $pomoc== 2)
					echo "<a href='./projekt_strona_glowna.php' class='btn btn-primary' style='margin:10px;'>Powrót do Strony Głównej Projektu</a>";
 ...
 <script>
    $(document).ready(function(){
        ...
        $('#repeater_form').on('submit', function(event){
            ...
            $.ajax({
                url:"przypisz_pracownikow_ajax.php",
                method:"POST",
                data:$(this).serialize(),
